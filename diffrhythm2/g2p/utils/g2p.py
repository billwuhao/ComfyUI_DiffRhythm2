...
# --- Replace EspeakBackend for Japanese with pyopenjtalk ---
class PyopenjtalkBackend:
    def phonemize(self, text, separator=None, strip=True, njobs=1):
        if isinstance(text, list):
            results = [pyopenjtalk.g2p(t, kana=False) for t in text]
        else:
            results = [pyopenjtalk.g2p(text, kana=False)]
        # pyopenjtalk.g2p() returns a string of phonemes separated by space
        return results

phonemizer_ja = PyopenjtalkBackend()
